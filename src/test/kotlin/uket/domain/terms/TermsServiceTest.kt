...
class TermsServiceTest :
    DescribeSpec({
        ...
        val termsService = TermsService(termsRepository, termsSignRepository, documentRepository)
        ...
        describe("agreeTerms") {
            context("필수 약관 1개가 있을 때") {
                ...
                it("유저가 동의 요청을 제출하면 TermSign 객체가 생성됨") {
                    val returnedTermSigns = termsService.agreeTerms(userId, userAnswerYes)
                    val termSigns = slot.captured
                    termSigns.size shouldBe 1
                    termSigns[0].userId shouldBe userId
                    termSigns[0].documentVersion shouldBe document.version
                ...
                it("유저가 거부 요청을 제출하면 에러 발생") {
                    val exception = shouldThrow<PublicException> { termsService.agreeTerms(userId, userAnswerNo) }
        ...
        describe("getAllActiveAndCheckRequiredByUser") {
            context("약관이 하나 있고, 유저가 해당 약관을 확인한 내역이 없는 경우") {
                ...
                every { termsSignRepository.findLatestByUserIdAndTermsIdsWithTerms(userId, listOf(1L)) } returns listOf()
                every { documentRepository.findLatestDocumentsByDocumentNos(listOf(terms.documentNo)) } returns listOf(document)

                it("해당 약관을 포함한 목록 제공") {
                    val checkRequiredTerms = termsService.getAllActiveAndCheckRequiredByUser(userId)
